  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const res = await fetch(
        `https://speedsearch.shubhamkakad10x.workers.dev/api/search?q=${input}`
      );
    ...
    <main className="h-screen w-screen grainy">
      <div className="flex flex-col gap-6 items-center pt-32 duration-500 animate-in animate fade-in-5 slide-in-from-bottom-2.5">
        <h1 className="text-5xl tracking-tight font-bold">
          ⚡ <span className="text-red-600">flash</span>Search
        </h1>
        <p className="text-zinc-600 text-lg max-w-prose text-center">
          A high-performance API built with Hono, Next.js and Cloudflare. <br />{" "}
          Type a country name below and get your flash results in miliseconds.
        </p>
